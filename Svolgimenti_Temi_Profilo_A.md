...
### Risposte-modello ai nove temi · Banca d'Italia 2026 (sei ufficiali + tre temi extra di allenamento)
...
## Svolgimento Tema 2.3 — Autenticazione forte e comunicazione sicura per l'accesso esterno a un portale di vigilanza

**Impostazione.** L'accesso esterno di soggetti vigilati a un portale che tratta dati riservati è un vettore critico: la maggior parte delle violazioni parte da credenziali compromesse, non da falle crittografiche negli algoritmi. Il problema va quindi affrontato su due piani distinti ma complementari: la sicurezza del canale (che i dati in transito siano riservati e integri) e la sicurezza dell'identità (che chi si autentica sia davvero chi dichiara di essere). Il principio guida è **Zero Trust**: nessuna fiducia implicita basata sulla provenienza della connessione, verifica esplicita ad ogni accesso.

**(a) TLS: combinazione di simmetrica e asimmetrica.** La crittografia asimmetrica è troppo lenta per cifrare l'intero flusso dati, ma è perfetta per risolvere il problema opposto della simmetrica: lo scambio sicuro della chiave su un canale non fidato. Nell'handshake TLS, il server presenta il proprio certificato; client e server derivano tramite uno scambio di chiavi (es. **ECDHE**, che garantisce *forward secrecy*) un segreto condiviso, da cui si deriva una **chiave di sessione simmetrica** (AES-GCM) che cifra tutto il traffico successivo. L'asimmetrica quindi non sostituisce la simmetrica: la **inizializza in modo sicuro**, dopodiché il grosso del lavoro è demandato alla simmetrica, molto più efficiente. La *forward secrecy* (chiave effimera per sessione) garantisce che la compromissione futura della chiave privata del server non permetta di decifrare traffico intercettato in passato.

**(b) Firma digitale, certificati, PKI.** La firma digitale (hash del messaggio cifrato con la chiave privata del firmatario, verificabile con la chiave pubblica) garantisce autenticità e non ripudio. Ma perché fidarsi di una chiave pubblica? Serve un **certificato X.509**, che lega una chiave pubblica a un'identità, firmato da una Autorità di Certificazione. La fiducia si estende lungo una catena: la CA radice (pre-installata nei trust store) firma CA intermedie, che firmano i certificati foglia. Il ciclo di vita include la **revoca** (un certificato compromesso va invalidato prima della scadenza) tramite **CRL** (liste scaricate periodicamente) o **OCSP** (verifica online in tempo reale) — quest'ultimo preferibile per rilevare compromissioni rapidamente.

**(c) Autenticazione forte e Zero Trust.** Per gli utenti esterni la sola password è insufficiente: va affiancata da un secondo fattore (MFA), idealmente di categoria diversa (qualcosa che l'utente ha/è, non solo sa). Tra le opzioni, **FIDO2/WebAuthn** è preferibile a OTP via SMS/app perché resistente al phishing: la chiave privata non lascia mai il dispositivo e la firma è vincolata al dominio richiedente, per cui un sito di phishing non può ottenere una firma valida anche se l'utente viene ingannato. Per la delega dell'autenticazione (es. integrazione con l'identity provider dell'intermediario) si usa **OAuth2** per l'autorizzazione e **OpenID Connect (OIDC)** come layer di autenticazione sopra OAuth2, evitando che il portale gestisca direttamente le credenziali di ogni intermediario. Il modello **Zero Trust** impone che ogni sessione sia verificata esplicitamente e periodicamente, con policy di accesso condizionale (es. rivalutazione se cambia rete o dispositivo).

**(d) Gestione sicura di credenziali e segreti.** Lato server, le password non vanno mai memorizzate in chiaro né con hash veloci: si usano KDF deliberatamente lente e memory-hard come **Argon2id**, con salt univoco per utente. Le chiavi private (es. quella del certificato TLS, o le chiavi di firma) non vanno mai incorporate nel codice o in file di configurazione in chiaro: vanno custodite in un **HSM** o in un **secret manager/vault** dedicato, con accesso controllato e audit, applicando una **rotazione periodica** delle chiavi e dei segreti applicativi per limitare la finestra di esposizione in caso di compromissione non rilevata.

**Esempio nel contesto.** Un portale esposto a intermediari esterni e trattante segnalazioni riservate è un bersaglio permanente per campagne di phishing e credential stuffing; l'adozione di FIDO2, OIDC per la delega dell'identità e HSM per la custodia delle chiavi è la traduzione pratica dei requisiti di gestione del rischio ICT e degli accessi richiesti da DORA e dei principi Zero Trust ormai standard per le infrastrutture finanziarie critiche.

**Rischi residui.** (1) **Adozione incompleta di MFA/FIDO2** da parte degli intermediari esterni (dipendenza da terzi per la sicurezza dell'endpoint). (2) **Compromissione di una CA intermedia** o mancata verifica OCSP tempestiva. (3) **Furto di sessione** post-autenticazione (session hijacking) se i token non sono adeguatamente protetti (short-lived, binding al dispositivo). (4) **Rischio operativo** nella gestione dell'HSM/vault (single point of failure se non ridondato). (5) **Ingegneria sociale** che aggira MFA meno robusti (SIM swap per OTP SMS) — da cui la preferenza per FIDO2.
...
## Svolgimento Tema 3.3 — Sistema multiagente per l'automazione dei controlli di vigilanza: conoscenza, ragionamento e dati sintetici

**Impostazione.** Automatizzare controlli di conformità che oggi richiedono competenza normativa specialistica multipla (contabile, AML, prudenziale) pone un problema che il solo machine learning statistico non risolve bene: molte regole di conformità sono esplicite, codificate in norme, e richiedono di essere applicate in modo deterministico e spiegabile — non "apprese" da esempi. Il progetto corretto è quindi ibrido: IA simbolica per la conoscenza codificata, IA statistica per i pattern che nessuna regola scritta può anticipare, orchestrati da un'architettura multiagente che rispecchi la specializzazione dei controlli reali.

**(a) IA simbolica vs induttiva.** L'IA **deduttiva/simbolica** rappresenta esplicitamente la conoscenza di dominio — tramite basi di regole (if-then), **ontologie** (tassonomie di concetti e relazioni tra entità normative) o grafi di conoscenza — e usa un **motore di inferenza** per derivare conclusioni logicamente valide da fatti e regole. È l'approccio corretto per codificare norme esplicite (es. soglie di segnalazione, requisiti di completezza di un flusso contabile): il risultato è deterministico, verificabile e spiegabile per costruzione, requisito essenziale in un controllo di conformità che deve poter essere motivato a un ispettore o a un giudice. L'IA **induttiva/statistica** (machine learning) apprende invece pattern da esempi storici, ed è insostituibile dove le regole non sono codificabili a priori — tipicamente il rilevamento di comportamenti anomali nuovi, non descrivibili con una regola esplicita nota in anticipo. I due approcci non sono alternativi ma complementari: le regole simboliche filtrano ciò che è normativamente definito, il ML individua ciò che è statisticamente anomalo ma non ancora normato.

**(b) Architettura multiagente.** Propongo agenti specializzati per dominio di controllo (un agente per i controlli contabili, uno per l'AML, uno per i controlli prudenziali), ciascuno incapsulando la propria base di regole e/o modello ML specifico, più un **agente orchestratore** che riceve la segnalazione, la instrada agli agenti pertinenti e aggrega gli esiti in un verdetto complessivo per l'analista umano. Rispetto a un sistema monolitico, questa architettura offre **modularità** (ogni dominio evolve la propria logica normativa senza toccare gli altri, importante perché le normative dei tre ambiti cambiano con cadenze indipendenti), manutenibilità e possibilità di scalare/aggiornare un singolo agente. Il costo è la necessità di gestire esplicitamente la **comunicazione tra agenti** (protocolli di messaggistica standardizzati) e la **coerenza degli esiti** quando più agenti producono valutazioni parzialmente in conflitto sulla stessa segnalazione — un problema di arbitraggio che l'orchestratore deve risolvere con regole di priorità esplicite, non lasciato all'emergenza.

**(c) Clustering e data visualization.** Sui grandi volumi di segnalazioni (big data), tecniche di apprendimento non supervisionato — **k-means** per segmentazioni nette, **DBSCAN** per cluster di forma arbitraria e rilevamento nativo di outlier, clustering **gerarchico** per esplorare relazioni a più livelli — permettono di individuare gruppi di comportamento omogenei e, per differenza, le osservazioni che non si adattano a nessun cluster (candidate anomalie), senza richiedere etichette pregresse: essenziale perché le violazioni nuove non sono per definizione etichettate. Gli esiti, per essere utili, devono essere resi *actionable* tramite **data visualization** (dashboard con proiezioni dimensionali ridotte, es. t-SNE/UMAP per visualizzare cluster in 2D, heatmap di anomalia nel tempo): la **sorveglianza umana** richiesta dall'AI Act per i sistemi ad alto rischio è effettiva solo se l'analista può ispezionare visivamente e comprendere perché un caso è stato segnalato, non se riceve un punteggio opaco.

**(d) Dati sintetici.** Per sviluppare e testare il sistema senza esporre dati reali riservati durante le fasi di sviluppo (privacy by design, minimizzazione), si generano **dati sintetici** che ne riproducano le proprietà statistiche. Tecniche: modelli generativi (**GAN**, variational autoencoder) per dati tabellari/serie complesse, tecniche basate su **copule statistiche** per preservare le correlazioni tra variabili, o oversampling sintetico (**SMOTE**) per arricchire le classi rare (utile proprio per i casi di frode, rari per natura). Il rischio è duplice e opposto: **bassa fedeltà statistica** (i dati sintetici non riproducono le correlazioni reali, e il modello addestrato su di essi non generalizza al mondo reale), oppure **eccessiva fedeltà** al punto di "memorizzare" record reali (in particolare con GAN sovra-addestrate), reintroducendo un rischio di *privacy leakage* che il dato sintetico avrebbe dovuto eliminare — va quindi validata sia l'utilità statistica sia l'assenza di memorizzazione (es. tramite test di *membership inference*) prima di considerare il dataset sintetico sicuro da usare o distribuire.

**Esempio nel contesto.** Per la Banca d'Italia, un sistema di questo tipo automatizza parte del lavoro oggi svolto da unità specialistiche distinte, mantenendo però — tramite la componente simbolica e la sorveglianza umana sull'esito — la tracciabilità e la motivabilità che un'autorità di vigilanza deve poter garantire; i dati sintetici permettono di sviluppare e validare il sistema in ambienti di test anche esterni (es. con fornitori) senza violare la riservatezza delle segnalazioni reali.

**Rischi residui.** (1) **Conflitto tra esiti di agenti diversi** non risolto in modo trasparente. (2) **Drift normativo**: le regole simboliche vanno aggiornate manualmente ad ogni modifica normativa, con rischio di disallineamento se il processo di manutenzione non è tempestivo. (3) **Falsi positivi**: cluster/anomalie statisticamente significative ma normativamente irrilevanti, che generano carico di lavoro per gli analisti. (4) **Dati sintetici** a bassa fedeltà che degradano le prestazioni in produzione, o al contrario troppo fedeli con rischio di privacy leakage residuo. (5) **Obblighi AI Act** (sistema ad alto rischio): necessità di documentazione tecnica, log e sorveglianza umana effettiva, non solo formale.
