...
Semantiche di consegna, il punto più insidioso. I broker garantiscono in genere una consegna at-least-once: un evento può arrivare più volte. Da qui due requisiti che in un tema non possono mancare: i consumatori devono essere idempotenti (rielaborare lo stesso evento non deve produrre effetti doppi — cruciale se l'evento è "addebita 100 euro"), e va gestito l'ordinamento dei messaggi, tipicamente garantito solo per chiave (es. per singolo conto) e non globalmente.
Perché si adotta questo stile — e quando invece no. I vantaggi sono forti: disaccoppiamento (i servizi evolvono e si rilasciano in autonomia), scalabilità orizzontale indipendente per ciascun servizio, resilienza (un consumatore lento o temporaneamente giù non blocca i produttori: gli eventi restano in coda), ed estensibilità (aggiungere un nuovo consumatore — per esempio un modulo antifrode che ascolta gli eventi di pagamento — non richiede di toccare i servizi esistenti). Il prezzo, però, è reale: consistenza eventuale anziché forte, complessità operativa e cognitiva molto maggiore, difficoltà di debugging e osservabilità (un flusso attraversa molti servizi in modo asincrono, per cui servono distributed tracing, logging correlato, monitoraggio degli SLO), e la gestione già citata di duplicati e ordinamento.
Il collegamento con il contesto Banca d'Italia — quello che fa la differenza a un colloquio o in un elaborato. Questo stile è eccellente per i domini "periferici" e reattivi: notifiche, aggiornamento di viste di reporting, alimentazione di sistemi di fraud detection o AML che ascoltano gli eventi di transazione, disaccoppiamento tra ingestione e elaborazione. Ma per il cuore del regolamento — dove un pagamento deve essere definitivo, atomico e fortemente consistente — la consistenza eventuale non basta: lì il ledger resta tipicamente ACID e fortemente consistente (CP nel teorema CAP), e l'event-driven si usa attorno a quel nucleo, non dentro. Saper indicare dove questo paradigma è appropriato e dove no è esattamente il tipo di ragionamento sui trade-off che la Commissione premia al Profilo A.
