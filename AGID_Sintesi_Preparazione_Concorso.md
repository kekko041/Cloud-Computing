...
*   **Documento Informatico:** Equiparato alla carta, se formato e conservato secondo le regole tecniche.
    *   *Firme (eIDAS/CAD):* **FES** (semplice, valore probatorio liberamente valutabile), **FEA** (avanzata, identificazione univoca del firmatario + integrità, senza certificato qualificato) e **FEQ/Firma Digitale** (qualificata: certificato qualificato + dispositivo sicuro, crittografia asimmetrica su hash del documento — piena efficacia probatoria equivalente alla firma autografa, art. 2702 c.c.). La FEQ è obbligatoria per atti con effetti giuridici pieni (contratti pubblici, provvedimenti formali); basta la FEA/FES per istanze informali o identificazione via SPID/CIE.
    *   *Dispositivi e ottenimento:* locale (smart card, token USB) o **remota** (chiave privata su HSM, accesso con 2FA), tramite uno dei prestatori di servizi fiduciari qualificati vigilati da AgID, previa identificazione (documento, SPID o CIE). La sola firma digitale non dà data certa: serve marca temporale o invio via PEC. Approfondimento completo in [`Sintesi_Firma_Digitale.md`](Sintesi_Firma_Digitale.md).
